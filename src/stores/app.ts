...
export const useAppStore = defineStore<'app', AppStore, AppGetters, AppActions>('app', {
  ...
  getters: {
    getTaskById() {
      return (taskId, userId) => this.tasks.find(
        ({ id, assigneeId, authorId }) => taskId === id && [authorId, assigneeId].some(x => x === userId)
      ) as Task;
    },
    getTasksByAuthorId() {
    ...
    getTasksByUserId() {
      return (userId) => this.tasks.filter(({ authorId, assigneeId }) => [authorId, assigneeId].some(x => x === userId));
    },
